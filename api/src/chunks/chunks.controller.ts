...
export class ChunksController {
  ...
  async list(@CurrentUser() user: AuthUser) {
    const chunks = await this.chunks.findAll(user.id);

    return serializeChunkResponseList(chunks);
  }

  @Post()
  async create(@CurrentUser() user: AuthUser, @Body() body: CreateChunkDto) {
  ...
  async getById(
    ...
    @Param() params: ChunkIdParamDto,
  ) {
    const id = validateChunkId(params.id);

    const chunk = await this.chunks.findOne(id, user.id);
    if (!chunk) {
  ...
  async update(
    @CurrentUser() user: AuthUser,
    @Param() params: ChunkIdParamDto,
    @Body() body: UpdateChunkDto,
  ) {
    const id = validateChunkId(params.id);
    validateUpdateChunkInput(body);

    const result = await this.chunks.update(
      id,
      {
        title: body.title?.trim(),
  ...
  async remove(
    ...
    const removed = await this.chunks.remove(id, user.id);
    if (!removed) {
      throw new NotFoundException(CHUNK_ERROR_MESSAGES.chunkNotFound);
    }
  }
